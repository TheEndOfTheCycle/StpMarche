...
public class GameScreen implements Screen {
    ...
    final String MUSIQUE3 = "Music/Rick.mp3";
    public Sound sonJeu;
    ...
    int mapWidth;
    private final String CARET1 = "Map/LatestMap.txt";
    private final String CARET2 = "Map/carte3.txt";
    ...
    private final int ENEMY_SPAWN_RATIO = 50;
    private int spawnType = 1;
    private final int BASIC_DEATH_REQ = 10;
    ...
    /**
     * Met à jour l'état du jeu en générant des ennemis volants et des soutiens de
     * feu
     * à des intervalles spécifiés.
     *
     * Cette méthode vérifie si le temps écoulé depuis le dernier spawn d'ennemi
     * volant ou de soutien de feu
     * dépasse les intervalles de génération définis. Si les conditions sont
     * remplies, elle génère de nouveaux ennemis.
     * La génération des ennemis est désactivée si le niveau du boss est actif.
     */
    public void update() {
        // Vérifie si le temps écoulé depuis le dernier spawn d'ennemi volant est
        // supérieur à l'intervalle de spawn
        ...
        // Vérifie si le temps écoulé depuis le dernier spawn de soutien de feu est
        // supérieur à l'intervalle de spawn
     ...
     *
     * Cette méthode parcourt les objets de la carte et les ennemis volants pour
     * vérifier
     * s'il y a des collisions avec le joueur. En cas de collision, des actions
     * spécifiques
     * sont effectuées, telles que la gestion des collisions, la réduction des
     * points de vie
     ...
     *
     * @param avion L'avion à vérifier s'il entre en collision avec les murs lors du
     *              spawn.
     * @return true s'il y a une collision avec un mur ou avec le joueur, sinon
     *         false.
     */
    ...
    /**
     * Vérifie s'il y a une collision entre un avion ennemi et les obstacles de la
     * carte.
     *
     * Cette méthode est utilisée pour détecter les collisions entre un avion ennemi
     * et les murs de la carte ainsi que le joueur. Elle prend en compte les
     * caractéristiques
     * spécifiques des avions ennemis, tels que l'invulnérabilité des avions
     * britanniques
     * aux obstacles de terrain.
     *
     * @param avion L'avion ennemi à vérifier s'il entre en collision avec les
     *              obstacles de la carte.
     * @return true s'il y a une collision avec un mur (pour les avions français),
     *         sinon false.
     */
    public boolean checkEnemyCollision(Plane avion) {
        ...
        for (Object object : objects) {
            ...
            if (object instanceof Wall) {
                ...
                if (avion.checkCollision(mur)) {
                    // Pour les avions britanniques (British), ajuste la position en cas de
    ...
    /**
     * Vérifie s'il est possible de faire spawn un bonus à une certaine position
     * sans collision avec les obstacles.
     *
     * Cette méthode est utilisée pour déterminer s'il est possible de faire spawn
     * un bonus à une position donnée
     * sans qu'il y ait de collision avec les murs de la carte. Elle retourne true
     * si la position spécifiée pour
     * le spawn du bonus est dégagée, sinon elle retourne false.
     *
     * @param buff Le bonus à vérifier pour le spawn.
     * @return true si la position spécifiée est dégagée pour le spawn du bonus,
     *         sinon false.
     */
        ...
        for (Object objet : objects) {
            ...
            if (objet instanceof Wall) {
                ...
                if (buff.collidesWith(mur)) {
                    // La position spécifiée pour le spawn du bonus n'est pas dégagée, retourne
                    // false
                    return false;
                }
            }
     ...
     *
     * Cette méthode est responsable du spawn aléatoire des bonus sur la carte, de
     * leur affichage,
     * de leur mise à jour et de leur interaction avec le joueur. Elle prend
     * également en charge la
     * vérification de la collision des bonus avec les obstacles de la carte et la
     * gestion des effets
    ...
    public void handleBuffSpawn() {
        ...
        // Vérifie si le temps écoulé depuis le dernier spawn de bonus dépasse le temps
        // de spawn spécifié
        if (elapsedTime - lastBuffSpawnTime > BUFF_SPAWN_TIME && buffitem == null) {
            // Si le temps de spawn est écoulé et qu'il n'y a pas de bonus actuellement sur
            // la carte
            ...
            while (!checkBuffSpawn(buffitem)) {
                // Si la position n'est pas dégagée, génère une nouvelle position aléatoire pour
                // le bonus
                buffitem = new Heart(MathUtils.random(100, 400), MathUtils.random(100, 400));
            }
     ...
     *
     * Cette méthode parcourt tous les projectiles présents sur la carte et vérifie
     * s'ils entrent en collision
     * avec des avions ennemis. Si un avion ennemi est touché par un projectile, ses
     * points de vie sont réduits.
     * Si les points de vie de l'avion ennemi atteignent 0, il est considéré comme
     * détruit et des actions spécifiques
    ...
    public void handleDamagedFlyingEnemy() {
        ...
        for (Projectile tire : projectiles) {
            ...
            for (Plane avion : FlyingEnemies) {
                // Vérification de la collision entre l'avion ennemi et le projectile, excluant
     ...
     *
     * Cette méthode vérifie si le joueur n'a plus de points de vie. Si tel est le
     * cas,
     * elle déclenche une explosion au niveau de la position actuelle du joueur et
     * marque
     ...
     *
     * Cette méthode est appelée lorsqu'une collision est détectée entre le joueur
     * et un obstacle
     * ou un ennemi. Elle déclenche une animation d'explosion au niveau de la
     * position actuelle du joueur
     * et marque le jeu comme terminé.
     */
    private void handlePlayerCollision() {
        // Produire l'animation d'explosion pour le joueur au niveau de sa position
        // actuelle
        explode(player.getX(), player.getY());
     ...
     *
     * Cette méthode parcourt tous les avions ennemis sur la carte et vérifie s'ils
     * entrent en collision
     * avec des obstacles ou le joueur. Si une collision est détectée, elle
     * déclenche une explosion au niveau
     * de la position de l'avion ennemi et met à jour son état pour indiquer qu'il a
     * été détruit.
     */
    ...
    /**
     * Gère les collisions entre les projectiles largués par les ennemis et les
     * défenses anti-aériennes.
     *
     * Cette méthode parcourt tous les projectiles largués par les ennemis sur la
     * carte et vérifie s'ils entrent
     * en collision avec les défenses anti-aériennes. Si une collision est détectée,
     * elle déclenche une explosion
     * à la position de la défense anti-aérienne, met à jour le score du jeu en
     * fonction de la valeur de l'anti-air,
    ...
    private void handleAntiAirCollisions() {
        ...
        for (int i = 0; i < projectiles.size; i++) {
            ...
            if (projectile instanceof Bomb) {
                // Parcours de tous les objets sur la carte pour trouver les défenses
                // anti-aériennes
                for (Object object : objects) {
                    ...
                    if (object instanceof AntiAir) {
                        ...
                        if (antiAir.collidesWith(projectile)) {
                            ...
                            // Si la carte actuelle est la carte 2, incrémentation du compteur de décès des
                            // défenses anti-aériennes
                            if (game.getCurrentMap() == 2) {
                                game.AntiAirDeath++;
                            }
     ...
     *
     * Cette méthode parcourt tous les murs de la carte et vérifie s'il y a une
     * collision
     * entre la bombe spécifiée et l'un des murs. Si une collision est détectée, la
     * méthode
    ...
    private boolean bombCollision(Bomb bomb) {
        // Parcours de tous les murs de la carte pour vérifier la collision avec la
        // bombe
        for (Object object : objects) {
            if (object instanceof Wall) {
                Wall wall = (Wall) object;
     ...
     *
     * Cette méthode parcourt tous les objets sur la carte et les dessine sur le lot
     * de rendu (batch)
     * associé. Elle prend en charge différents types d'objets tels que les murs,
     * les zeppelins, les défenses
    ...
    /**
     * Crée un nouveau projectile de type bullet à partir de la position actuelle du
     * joueur.
     * 
     * Cette méthode crée un nouveau projectile de type bullet à partir de la
     * position actuelle du joueur
    ...
    /**
     * Crée un nouveau projectile de type bomb à partir de la position actuelle du
     * joueur.
     * 
     * Cette méthode crée un nouveau projectile de type bomb à partir de la position
     * actuelle du joueur
     ...
     * 
     * Cette méthode parcourt la liste des projectiles et dessine chacun d'eux sur
     * le lot de rendu (batch).
     ...
     * 
     * Cette méthode génère un nouvel ennemi volant en fonction de la carte actuelle
     * du jeu. Sur la première carte, uniquement des ennemis de type French sont
     * générés. Sur la deuxième carte, des ennemis de type French et British peuvent
     * être générés en fonction d'un ratio défini. L'ennemi est créé avec des
     * coordonnées aléatoires sur l'axe Y et ajouté à la liste des ennemis volants.
     */
    public void createFlyingEnemy() {
        ...
        if (game.getCurrentMap() == 1) {
            ...
            // Sur la deuxième carte, des ennemis de type French et British peuvent être
            // générés en fonction d'un ratio défini
        ...
        // Ajout de l'ennemi à la liste des ennemis volants et mise à jour du temps de
        // spawn
     ...
     * 
     * Cette méthode génère un nouvel ennemi de soutien (artillerie) avec des
     * coordonnées aléatoires sur l'axe X et en haut de l'écran sur l'axe Y.
     * L'ennemi est créé avec des dimensions spécifiées et une vitesse définie, puis
     * ajouté à la liste des ennemis de soutien. Le temps de la dernière apparition
     * de coquille est également mis à jour.
     */
     ...
     * 
     * Cette méthode permet de dessiner un ennemi volant en utilisant la classe
     * ShapeRenderer pour dessiner sa forme, puis en appelant la méthode de dessin
     ...
     * 
     * Cette méthode met à jour la position de chaque ennemi de soutien dans la
     * liste des ennemis de soutien. Si un ennemi de soutien sort de l'écran, il est
     * supprimé de la liste. Ensuite, pour chaque ennemi de soutien restant, la
     * méthode appelle la fonction de dessin appropriée.
     */
     ...
     * Dessine un ennemi volant.
     * 
     * Cette méthode permet de dessiner un ennemi volant en utilisant le lot de
     * rendu (batch). Elle appelle la méthode de dessin de l'ennemi pour chaque
     * ennemi dans la liste des ennemis de soutien.
     * 
     * @param avion L'ennemi à dessiner.
     */
    public void SpawnFlyingEnemy(Plane avion) {
     ...
     * 
     * Cette méthode gère les mouvements et l'état des ennemis volants. Elle
     * parcourt la liste des ennemis volants et met à jour chaque ennemi. Si un
     * ennemi est endommagé et atteint 0 points de vie, il est supprimé de la liste
     * et une explosion est déclenchée à sa position. De plus, si un ennemi atteint
     * la limite gauche de l'écran, il est également supprimé de la liste. Ensuite,
     * la méthode parcourt à nouveau la liste pour afficher chaque ennemi volant à
     * l'écran.
     */
    public void UpdateFlyingEnemy() {
        ...
        while (iter.hasNext()) {
            ...
            // Si l'ennemi est détruit (0 points de vie), il est supprimé de la liste et une
            // explosion est déclenchée
            if (avion.getHp() == 0) {
                explode(avion.getX(), avion.getY());
                iter.remove();
            }
     ...
     * 
     * Cette méthode parcourt la liste des explosions et les affiche à l'écran. Elle
     * met également à jour la position de chaque explosion en fonction du temps
     * écoulé depuis la dernière mise à jour. Si une explosion est terminée
     * (c'est-à-dire si son animation est terminée), elle est supprimée de la liste.
     * 
     ...
     * 
     * Efface l'écran, dessine la carte, met à jour le jeu en cours (si le jeu n'est
     * pas terminé), rend le gameplay (y compris les ennemis et les objets), met à
     * jour et dessine les projectiles, gère les collisions avec les ennemis, rend
     * les explosions (si le jeu n'est pas terminé), et affiche l'écran de fin de
     * partie si le joueur a perdu ou a accompli les conditions de victoire pour la
     * carte actuelle.
     * 
     ...
     * 
     * Met à jour et dessine le joueur, gère la génération et l'affichage des buffs,
     * affiche le score et l'objectif en cours (nombre d'ennemis tués ou
     * d'anti-aériens détruits), met à jour et dessine le boss si présent sur la
     * carte, et vérifie les collisions du joueur avec les différents éléments du
     * jeu.
     */
     ...
     * 
     * Affiche une animation d'explosion centrée sur la position du joueur, puis
     * affiche l'écran de fin de partie et enregistre le score dans un fichier. Une
     * fois l'animation terminée, arrête la lecture de la musique de jeu et passe à
     ...
     * 
     * Affiche l'écran de victoire et enregistre le score dans un fichier. Passe à
     * l'écran de victoire une fois l'enregistrement terminé.
     */
